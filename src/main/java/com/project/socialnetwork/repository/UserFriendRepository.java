...
public interface UserFriendRepository extends JpaRepository<UserFriend,Long> {
    ...
    @Query("""
                select uf.id from UserFriend uf
                where (uf.firstUser.id=:userId or uf.secondUser.id=:userId) and uf.hasAccepted=true
            """)
    ...
    @Query("""
                ...
                where (uf.firstUser = :fId and uf.secondUser= :sId) or(uf.firstUser = :sId and uf.secondUser= :fId)
            """)
    Optional<UserFriend> getUserFriendBy2UserIds(Long fId,Long sId);
